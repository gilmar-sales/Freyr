    ...
    switch (compCount)
    {
        case 8:
            return registry.CreateEntity(
                Position {}, Velocity {}, Health { .value = 100.f }, Damage { .value = 10.f },
                TagA {}, TagB {}, TagC {}, TagD {});
        case 7:
            return registry.CreateEntity(Position {}, Velocity {}, Health { .value = 100.f },
                                         Damage { .value = 10.f }, TagA {}, TagB {}, TagC {});
        case 6:
            return registry.CreateEntity(Position {}, Velocity {}, Health { .value = 100.f },
                                         Damage { .value = 10.f }, TagA {}, TagB {});
        case 5:
            return registry.CreateEntity(Position {}, Velocity {}, Health { .value = 100.f },
                                         Damage { .value = 10.f }, TagA {});
        case 4:
            return registry.CreateEntity(
                Position {}, Velocity {}, Health { .value = 100.f }, Damage { .value = 10.f });
        case 3:
...
static void BM_Mutation_Each_Sync(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        registry.CreateMutation()->Each([](Position& pos, Velocity& vel) {
            pos.x += vel.x;
            pos.y += vel.y;
            pos.z += vel.z;
        });
...
static void BM_Query_Transform(benchmark::State& state)
{
    ...
    {
        auto query   = registry.CreateQuery();
        auto results = query->Transform([](Position& pos, Velocity& vel) { return pos.x + vel.x; });
...
static void BM_Query_Map(benchmark::State& state)
{
    ...
    {
        auto query   = registry.CreateQuery();
        auto results = query->Map([](Position& pos, Velocity& vel) { return pos.x + vel.x; });
        benchmark::DoNotOptimize(results.size());
    }
...
static void BM_Query_Reduce(benchmark::State& state)
{
    ...
    {
        auto  query = registry.CreateQuery();
        float sum   = query->Reduce(
            [](float acc, Position& pos, Velocity& vel) { return acc + pos.x + vel.x; }, 0.0f);
...
static void BM_Mutation_EachAsync(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        {
            registry.CreateMutation()->EachAsync([](Position& position, Velocity& velocity) {
...
static void BM_Mutation_EachAsync_FilteredByComponent(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        for (std::size_t i = 0; i < mutationCount; ++i)
        {
            registry.CreateMutation()->EachAsync([](Position& position, Velocity& velocity, TagA&) {
                position.x += velocity.x;
                position.y += velocity.y;
                position.z += velocity.z;
            });
...
static void BM_MixedWorkload(benchmark::State& state)
{
    ...
    {
        auto mutation = registry.CreateMutation();
        mutation->EachAsync([](Position& pos, Velocity& vel) { pos.x += vel.x * 0.5f; });
        registry.ExecuteTasks();
...
static void BM_Mutation_WithLabel_Profiling(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        mutation->Each([](Position& pos, Velocity& vel) { pos.x += vel.x; });
    }
...
BENCHMARK(BM_Query_Count)->Arg(10000)->Arg(100000)->Arg(1000000)->Unit(benchmark::kNanosecond);

BENCHMARK(BM_Query_FindUnique)->Arg(10000)->Arg(100000)->Arg(1000000)->Unit(benchmark::kNanosecond);
...
BENCHMARK(BM_Query_Transform)->Arg(10000)->Arg(100000)->Arg(1000000)->Unit(benchmark::kMicrosecond);

BENCHMARK(BM_Query_Map)->Arg(10000)->Arg(100000)->Arg(1000000)->Unit(benchmark::kMicrosecond);

BENCHMARK(BM_Query_Reduce)->Arg(10000)->Arg(100000)->Arg(1000000)->Unit(benchmark::kMicrosecond);
...
BENCHMARK(BM_MixedWorkload)->Arg(10000)->Arg(100000)->Arg(1000000)->Unit(benchmark::kMillisecond);
